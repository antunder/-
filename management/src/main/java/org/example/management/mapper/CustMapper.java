...
public interface CustMapper {
    ...
    //新增客户
    @Insert("insert into cust ( name,address, kind, gender,create_time) " +
            "values ( #{name},#{address}, #{kind},#{gender},#{createTime});")
    void insert(Cust cust);
    //修改客户信息
    void update(Cust cust);
    //根据id查询客户信息
    @Select("select id, name,address, kind, geddner,create_time " +
            "from cust " +
            "where id = #{id}")
    Cust findById(Integer id);
    //获取所有客户
    @Select("select id, name,address, kind, geddner,create_time from cust ")
    List<Cust> getAll();
    //批量删除
